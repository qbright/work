 ...
 *
 * Changelog  
 *  $Log: IntArrayParser.java,v $
 *  Revision 1.3  2007/12/22 19:35:14  remjohn
 *  Improve DataParser framework
 *
 *  Revision 1.2  2007/10/11 11:44:22  remjohn
 *  Added cast from long[]
 *
...
public class IntArrayParser implements Parser<int[]> {
    ...
    public int[] parse(String value) throws ParseException {
        if (value == null || value.length() == 0)
            return new int[0];
        return cast(new ArrayParser<Integer>(Integer.class).parse(value));
    }
    ...
    public int[] cast(Object value) {
        ...
        Integer[] temp = new ArrayParser<Integer>(Integer.class).cast(value);
        if (temp == null || temp.length == 0)
            return new int[0];
        int[] result = new int[temp.length];
        for (int i = 0; i < result.length; i++)
            result[i] = (temp[i] != null ? temp[i].intValue() : 0);
        return result;
    }
