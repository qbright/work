 ...
 *
 *  Revision 1.3  2007/10/05 01:16:42  remjohn
 *  Fix override
 *
 ...
 * @model
 */
public class DimensionParser implements Parser<Point2D> {
    ...
    /**
     * Parse a string as a Point2D object
     * @param resultClass
     * @param numberClass
     * @param values
     * @return
    ...
    protected Point2D parse(Class<? extends Point2D> resultClass, Class<? extends Number> numberClass, String[] values) throws ParseException {
        ...
        if (resultClass == null) {
            resultClass = classes.get(NumberArrayParser.getPreferredNumberClass(values));
            if (resultClass == null)
                resultClass = Point2D.Double.class;
        }

        try {
            con = NumberArrayParser.getPreferredConstructor(resultClass, values.length);
        ...
        if (con == null) {
            try {
                con = NumberArrayParser.getContructor(resultClass, NumberArrayParser.getPreferredNumberClasses(values));
     ...
     */
    @Override
    public Point2D parse(String value) throws ParseException {
