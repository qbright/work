...
public class XMLParser implements Parser<Document> {
     ...
     */
    @Override
    public Document parse(String value) throws ParseException {
        value = Utils.normalize(value);
        if (value == null)
            return null;
        try {
            return DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(value);
    ...
    public static String stream(Document xml) throws TransformerFactoryConfigurationError, TransformerException, IOException {
        if (xml == null)
            return null;
        return XMLUtils.toXML(xml);
     ...
     */
    @Override
    public Document cast(Object value) {
        if (value instanceof Document)
        ...
        if (value instanceof Element) {
            try {
                ...
                Element root = doc.createElement(el.getNodeName());
                XMLUtils.copyAttributes(el, root);
                XMLUtils.copyStructure(el, root);
                return (Document) value;
            } catch (ParserConfigurationException e) {
